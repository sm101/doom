...
(defun my/ensure-combined-dictionary ()
  "Combine spell-fu's EN and SR-Latin word lists for company-ispell.
Uses spell-fu's own API to locate word list files; no hardcoded paths."
  (let* ((dict-dir (expand-file-name "dicts" doom-data-dir))
         (combined (expand-file-name "combined-words.txt" dict-dir))
         (en-file (spell-fu--words-file (spell-fu-get-ispell-dictionary "default")))
         (sr-file (spell-fu--words-file (spell-fu-get-ispell-dictionary "sr-Latn"))))
    (when (and (file-exists-p en-file) (file-exists-p sr-file))
      (unless (file-exists-p combined)
        (make-directory dict-dir t)
        (with-temp-buffer
          (insert-file-contents en-file)
          (goto-char (point-max))
          (insert-file-contents sr-file)
