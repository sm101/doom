...
spell-fu already builds and caches word lists from the ispell dictionaries (see above).
We simply combine those cached =.txt= files rather than invoking aspell again.
No direct aspell dependency here — any ispell-compatible backend works.
The combined file is generated on first spell-fu activation and cached in =doom-data-dir=.

#+begin_src elisp
(defun my/ensure-combined-dictionary ()
  "Combine all active spell-fu word lists for company-ispell.
Derives file paths from spell-fu-dictionaries — no hardcoded dictionary names."
  (let* ((dict-dir (expand-file-name "dicts" doom-data-dir))
         (combined (expand-file-name "combined-words.txt" dict-dir))
         (word-files (delq nil
                           (mapcar (lambda (dict)
                                     (let ((f (spell-fu--words-file dict)))
...
(after! (spell-fu company)
  (add-hook 'spell-fu-mode-hook
            (lambda ()
              (setq company-ispell-dictionary (my/ensure-combined-dictionary)))
            t))  ; append — must run after the sr-Latn dictionary hook adds its entry
#+end_src
