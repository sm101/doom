...
#+end_src
* Company, combine English and Serbian
You need to install hunspell and to download English and Serbian dictionaries to ~/Library/Spelling.

English -> https://wordlist.aspell.net/dicts/

Serbian -> https://github.com/grakic/hyphen-sra

#+begin_src elisp
(defun my/ensure-combined-dictionary ()
  "Generate combined EN+SR dictionary if it doesn't exist."
  (let* ((dict-dir (expand-file-name "dicts" doom-data-dir))
         (combined (expand-file-name "combined-words.txt" dict-dir))
         (hunspell-dir (expand-file-name "~/Library/Spelling"))
         (en-dic (expand-file-name "en_US.dic" hunspell-dir))
         (sr-dic (expand-file-name "sr-Latn.dic" hunspell-dir)))
    (unless (file-exists-p combined)
      (make-directory dict-dir t)
      (message "Generating combined dictionary...")
      (shell-command
       (format "tail -n +2 %s | sed 's|/.*||' > /tmp/en.txt && \
                tail -n +2 %s | sed 's|/.*||' > /tmp/sr.txt && \
                cat /tmp/en.txt /tmp/sr.txt | sort -u > %s"
               (shell-quote-argument en-dic)
               (shell-quote-argument sr-dic)
               (shell-quote-argument combined)))
      (message "Combined dictionary generated at %s" combined))
    combined))
